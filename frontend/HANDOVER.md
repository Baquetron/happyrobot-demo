...
- **(this commit)** — `public/happyrobot-wordmark.svg` actually contains unrelated shipping-company artwork (CMA CGM / Ryder paths), not HappyRobot text. Stop using it. Badge now renders the mark SVG + a styled `<span>HappyRobot</span>` in semibold white. Wrapped the badge in an `<a href="https://happyrobot.ai">` for affiliation linking. The wordmark file is left in `public/` untouched in case anything else references it.
